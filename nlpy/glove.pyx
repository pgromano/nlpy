...
cdef object VOCAB_MAP = {
    "6B-50d": S3_BUCKET + "6B/vocab.json",
    "6B-100d": S3_BUCKET + "6B/vocab.json",
    "6B-200d": S3_BUCKET + "6B/vocab.json",
    "6B-300d": S3_BUCKET + "6B/vocab.json",
    "27B-25d": S3_BUCKET + "27B/vocab.json",
    "27B-50d": S3_BUCKET + "27B/vocab.json",
    "27B-100d": S3_BUCKET + "27B/vocab.json",
    "27B-200d": S3_BUCKET + "27B/vocab.json",
    "42B-300d": S3_BUCKET + "42B/vocab.json",
    "840B-300d": S3_BUCKET + "840B/vocab.json",
}

cdef object VECTOR_MAP = {
    "6B-50d": S3_BUCKET + "6B/vectors-50d.npy",
    "6B-100d": S3_BUCKET + "6B/vectors-100d.npy",
    "6B-200d": S3_BUCKET + "6B/vectors-200d.npy",
    "6B-300d": S3_BUCKET + "6B/vectors-300d.npy",
    "27B-25d": S3_BUCKET + "27B/vectors-25d.npy",
    "27B-50d": S3_BUCKET + "27B/vectors-50d.npy",
    "27B-100d": S3_BUCKET + "27B/vectors-100d.npy",
    "27B-200d": S3_BUCKET + "27B/vectors-200d.npy",
    "42B-300d": S3_BUCKET + "42B/vectors-300d.npy",
    "840B-300d": S3_BUCKET + "840B/vectors-300d.npy",
}
...
cdef class GloVe:
    ...
    def similarity(self, a, b):
        ...
        the GloVe vocabulary. Cosine similarity scores are re-scaled to be from 
        0 to 1.
    ...
    def from_file(cls, path, vocab_path=None):
        """ Create a GloVe Method from file
        ...
        if path in VOCAB_MAP:
            ...
            vectors_output = output_path / f'vectors-{embedding_size}.npy'
            ...
            if path.exists():
                ...
                for line in file:
                    ...
                    vectors.append(np.array(tokens[1:], dtype=np.float32))
                    index += 1
    ...
    def to_torch(self, path=None, requires_grad=True):
        """ Convert GloVe Embeddings to PyTorch Embeddings
    ...
    def __repr__(self):
        return "GloVe(vocab_size={}, embedding_size={})".format(
            self.vocab_size,
            self.embedding_size
        )
